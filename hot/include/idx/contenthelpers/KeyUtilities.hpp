...
template<> __attribute__((always_inline)) inline auto toBigEndianByteOrder<const char*>(const char * const & key) {
	return key;
};
...
template<> constexpr inline __attribute__((always_inline)) size_t getMaxKeyLength<long unsigned int>() {
...
inline auto toFixSizedKey(long unsigned int const & key) {
	return key;
}
...
template<> inline uint8_t const* interpretAsByteArray<uint64_t>(const uint64_t & key) {
	return reinterpret_cast<uint8_t const *>(key);
}

template<> inline uint8_t const* interpretAsByteArray<std::array<unsigned char,255ul>>(std::array<unsigned char,255ul> const & key) {
	return key.data();
}
